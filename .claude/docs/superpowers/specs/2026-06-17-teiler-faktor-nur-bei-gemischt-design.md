...
export function effectiveTeilerFaktor(competitionDisciplineId: string | null, faktor: number): number {
  return competitionDisciplineId === null ? faktor : 1
}
```

Alle Aufrufer ziehen den Faktor künftig durch diese Funktion. Der echte `teilerFaktor` bleibt unverändert in DB und Query — nur seine *Anwendung* wird konditioniert.
