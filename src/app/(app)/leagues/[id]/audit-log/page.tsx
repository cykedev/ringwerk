...
import { getAuditLogsByLeague } from "@/lib/auditLog/queries"
...
export default async function LeagueAuditLogPage({ params }: Props) {
  const { id } = await params

  const session = await getAuthSession()
  if (!session) redirect("/login")
  if (session.user.role !== "ADMIN") redirect(`/leagues/${id}/schedule`)

  const [league, entries] = await Promise.all([getLeagueById(id), getAuditLogsByLeague(id)])
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {league.discipline.name} · Protokoll
            </p>
          </div>
