...
interface Props {
  ...
  /** Effective ScoringType for rings input — derived from scoringMode + discipline in the page. */
  scoringType: ScoringType
  /** Effective teilerFaktor for live corrected-teiler hint (already resolved by page). */
  teilerFaktor: number
}
...
function deriveMatchStatus(
  ...
  for (const s of series) {
    ...
    if (s.isTiebreak) {
      ...
      // correctedTeiler = teiler × effectiveTeilerFaktor;
      // In the client we don't know the per-series teilerFaktor, so we use 1 as a fallback
      // (the factor is only applied for mixed competitions, where it's already stored in ringteiler).
      // For display purposes the key is just the duelOutcome comparison — both sides use the same factor.
...
/** Returns the sorted list of completed duel numbers (regular only). */
function completedDuelNumbers(
  ...
  for (const n of seenHome) {
    if (seenAway.has(n)) complete.add(n)
  }
  return Array.from(complete).sort((a, b) => a - b)
}

/** Returns Stechschuss pairs (both sides present). */
...
function ShooterInput({
  ...
  // Live corrected-teiler hint: only shown for mixed competitions (factor ≠ 1).
...
export function BestOfMatchCard({
  matchupId,
  homeParticipant,
  awayParticipant,
  series,
  ...
  // Stechschuss input state (single decimal shot per side)
  ...
  const winnerName = winnerId === homeId ? homeName : awayName
    ...
    <>
      ...
      <Card>
        <CardHeader className="px-4 pb-2 sm:px-6">
          <div className="flex items-center justify-between gap-2">
            ...
            {isComplete && winnerId && (
              <Badge
                variant="outline"
                className="gap-1 text-xs border-emerald-500/60 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
              >
        ...
        <CardContent className="space-y-3 px-4 sm:px-6">
          ...
          {/* Input section — only shown when canManage and not complete */}
          {canManage && matchStatus.kind === "in_progress" && (
          ...
          {/* Stechschuss input */}
          {canManage && matchStatus.kind === "needs_tiebreak" && (
            <div className="space-y-3 rounded-md border bg-muted/20 px-3 py-3">
              <p className="text-xs font-medium text-muted-foreground">Stechschuss eintragen</p>
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label htmlFor="home-shot" className="text-xs text-muted-foreground">
                      {homeName}
                    </Label>
                    <Input
                      id="home-shot"
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="away-shot" className="text-xs text-muted-foreground">
                      {awayName}
                    </Label>
                    <Input
                      id="away-shot"
          ...
          {/* Complete state — undo button */}
          {canManage && isComplete && (duelNumbers.length > 0 || stechschussNumbers.length > 0) && (
